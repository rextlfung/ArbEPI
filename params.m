...
% Extra margin added to Pulseq rfRingdownTime to prevent RF-gradient overlap.
% Larger than psd_rf_wait below to be conservative across GE scanner variants.
rf_ringdown_margin = 200e-6; % s

% Create Siemens/pulseq MRI system
% rfRingdownTime is extended by rf_ringdown_margin to ensure that the
% subsequent wait pulse (delay block) doesn't overlap with the true RF
% ringdown time (54 us).
sys = mr.opts('maxGrad', 50, 'gradUnit','mT/m', ...
              'maxSlew', 160, 'slewUnit', 'T/m/s', ...
              'rfDeadTime', 100e-6, ...
              'rfRingdownTime', 60e-6 + rf_ringdown_margin, ...
              'adcDeadTime', 20e-6, ...
              'adcRasterTime', 2e-6, ...
              'rfRasterTime', 2e-6, ...
              'gradRasterTime', 4e-6, ...
